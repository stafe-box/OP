...
def draw_simetric_branches(point, angle, length=100, set_day=None):
    ...
    vector = sd.get_vector(
        start_point=point, angle=angle, length=length, width=5)
    ...
    if length < 30:
        if set_day == 'morning':
            ...
            sd.circle(next_point, 4, sd.COLOR_ORANGE, width=0)
        elif set_day == 'afternoon':
            sd.circle(next_point, 7, sd.COLOR_WHITE, width=0)
            sd.circle(next_point, 2, sd.COLOR_ORANGE, width=0)
        elif set_day == 'evening':
            sd.circle(next_point, 7, sd.COLOR_WHITE, width=0)
            sd.circle(next_point, 3, sd.COLOR_ORANGE, width=0)
        else:
            sd.circle(next_point, 7, sd.COLOR_ORANGE, width=0)
...
def draw_branches(point, angle, length=100):
    ...
    vector = sd.get_vector(
        start_point=point, angle=angle, length=length, width=2)
    vector.draw()
...
if __name__ == '__main__':
    ...
    draw_simetric_branches(
        point=root_point, angle=root_angle, length=70, set_day=True)
