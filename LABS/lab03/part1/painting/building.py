...
def draw_ground():
    # рисуем землю
    sd.rectangle(sd.get_point(0, 0), sd.get_point(
        sd.resolution[0], 40), sd.COLOR_WHITE)
    sd.rectangle(sd.get_point(0, 0), sd.get_point(
        sd.resolution[0], 30), sd.COLOR_DARK_ORANGE)
...
def draw_house():
    ...
    sd.circle(sd.get_point(building_start_point.x+1 + building_size[0]//2,
              ...
              30, color=sd.COLOR_DARK_BLUE, width=0)
